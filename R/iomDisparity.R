...
iomDisparity.glm <- function(m, x, y, index, race,
  ...
  for(i in 1:length(race2$H_rank)) {
    for(j in 1:length(race1$H_rank)) {
      count <- count + 1
      if(race2$H_rank[i] <= race1$H_rank[count]) { 
        race2$H_i[i] <- race1$H_i[count]
        break
      } 
    }
  }
  ...
  new_expected_y <- mean(new_y_i)
  old_expected_y <- mean(old_y_i)
